/**
 * @file payment_management_system.cpp
 * @author Krishna Chauhan
 * @date 2024-10-29
 *
 * @brief This file contains the implementation of a Payroll Management System.
 *
 * The system allows for the management of employee records, including insertion,
 * editing, deletion, and display of employee details. The employees are stored
 * in a Binary Search Tree (BST) based on their unique codes for efficient
 * insertion and retrieval. Additionally, a hash map is used for quick search
 * operations based on employee codes.
 *
 * Classes:
 * - Employee: Represents an employee with attributes like name, code, designation,
 *   experience, age, salary, and loan status.
 * - BSTNode: Represents a node in the Binary Search Tree, containing an Employee
 *   object and pointers to left and right child nodes.
 * - BST: Manages the Binary Search Tree operations such as insertion and in-order
 *   traversal to retrieve all employees.
 *
 * Functions:
 * - insertEmployee: Prompts the user to enter employee details and inserts a new
 *   employee into the BST and hash map.
 * - editEmployee: Allows the user to edit the details of an existing employee.
 * - deleteEmployee: Deletes an employee from the hash map and BST.
 * - displayEmployees: Displays the list of all employees in the BST.
 *
 * The main function provides a menu-driven interface for the user to interact
